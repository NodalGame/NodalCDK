use lambda_http::{Body, run, service_fn, tracing, Error, Request, Response};
...
async fn handler(event: Request) -> Result<Response<Body>, Error> {
    ...
    // println!("got event headers: ");
    // event.headers().iter().for_each(|(k, v)| {
    //     println!("{}: {}", k, v.to_str().unwrap());
    // });

    tracing::info!("You're in the handler now.");

    let response = Response::builder()
        .status(200)
        .header("content-type", "text/html")
        .body("Hello OAuth!".into())
        .map_err(Box::new)?;
